...
def injecter_clefs_dropbox(writerdeck_path, app_key, app_secret):
    """Injecte les clefs Dropbox dans writerdeck.py après téléchargement."""
    try:
        with open(writerdeck_path, 'r') as f:
            content = f.read()
        content = content.replace(
            'DROPBOX_APP_KEY    = "COLLE_TON_APP_KEY_ICI"',
            f'DROPBOX_APP_KEY    = "{app_key}"'
        )
        content = content.replace(
            'DROPBOX_APP_SECRET = "COLLE_TON_APP_SECRET_ICI"',
            f'DROPBOX_APP_SECRET = "{app_secret}"'
        )
        with open(writerdeck_path, 'w') as f:
            f.write(content)
        return True
    except Exception:
        return False
...
def appliquer(cfg, clavier, dropbox_cfg):
    ...
    # 5. (clefs Dropbox injectées après téléchargement)
    ...
    # Injection des clefs Dropbox dans writerdeck.py
    if dropbox_cfg:
        writerdeck_path = os.path.join(writerdeck_dir, "writerdeck.py")
        if injecter_clefs_dropbox(writerdeck_path, dropbox_cfg["app_key"], dropbox_cfg["app_secret"]):
            ok("Clefs Dropbox injectées dans writerdeck.py")
        else:
            warn("Injection échouée — renseigner manuellement dans writerdeck.py")
...
def resume(cfg, clavier, dropbox_cfg):
  ...
  Dropbox       : {"✓ (autorisation au 1er lancement de writerdeck)" if dropbox_cfg else "—"}

  Scripts téléchargés dans ~/writerdeck/ depuis GitHub
""")
    if dropbox_cfg:
        print(f"  {YELLOW}Dropbox — étape finale :{RESET}")
        print("    Lance writerdeck → il proposera l'autorisation au démarrage.")
        print()
